...
async def render(p_rotate: str, visualization_mode: str, start_frame: int, duration: int, background_tasks: BackgroundTasks, bvh_file: UploadFile = File(...), audio_file: Optional[UploadFile] = File(None)):
	bvh_file_uri = await save_tmp_file(bvh_file)
	audio_file_uri = None
	if audio_file is not None:
		audio_file_uri = await save_tmp_file(audio_file)
	task = celery_workers.send_task("tasks.render", args=[bvh_file_uri, audio_file_uri, p_rotate, visualization_mode, start_frame, duration], kwargs={})
